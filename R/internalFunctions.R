...
.denominator <- function(x, ref, refinfo, split = NULL, 
  ...
  deno <- function(xx, ref, testtips, c.out,
                   sub_coverage){
    nod <- xx
    ...
    tmp <- ape::extract.clade(ref, node = xx)
    tmp <- tmp$tip.label
    ...
    for(d in 1:length(des)){
      dd <- des[d]
      # to collect all tips of the entire clade
      alltips <- c()
      
      # filters with node validation 
      # by the present of one sp in each of A/B clade
      # (+2 when valid for both des nodes)
      if(dd <= rlen){ # if tips
        rr <- ref$tip.label[dd]
        # valid when having this one sp (due to tip)
        if(any(testtips == rr))
          record <- record +1
        alltips <- c(alltips, rr)
      } # if tips
      if(dd > rlen){ # if internal nodes
        sref <- ape::extract.clade(ref, node = dd)
        stips <- sref$tip.label
        match <- stips %in% testtips 
        if(any(match))
          record <- record +1
        alltips <- c(alltips, stips)
      } # if internal node
    }
    ...
    if(!is.null(sub_coverage)){
      # (+1 when valid)
      match <- alltips %in% testtips
      if(sub_coverage <= 1){
        # valid when > % sp in each clade
        pt <- length(which(match))
        pa <- length(testtips)
        per <- pt / pa
        if(per >= sub_coverage)
          record <- record +1
        ...
        if(length(which(match)) >= sub_coverage)
          record <- record +1
      }
    } else {
    ...
    if(c.out){ 
      anc <- phangorn::Ancestors(ref, node = nod, 
                                 type = "parent");anc
      ades <- phangorn::Descendants(ref, node = anc, 
                                    type = "children");ades
      ades <- ades[-which(ades == nod)]
      
      if(ades <= rlen){ # if tips
        rr <- ref$tip.label[ades]
        if(any(testtips == rr))
          record <- record +1
      }
      if(ades > rlen){ # if internal nodes
        sref <- ape::extract.clade(ref, node = ades)
        stips <- sref$tip.label
  ...
  rres <- lapply(allnodes, function(xx) 
    deno(xx, ref, testtips, c.out, sub_coverage))
  res <- unlist(rres)
  names(res) <- allnodes
  return(res)
} # end of .denominator
